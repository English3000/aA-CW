...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(tracks.song)/*, COUNT(tracks.song LIKE '%Heart%')*/ /*in WHERE*/ AS heart_songs --error
    FROM
      albums
    JOIN
      tracks ON albums.asin = tracks.album
...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT
      t1.song, COUNT(t1.song) --error
    FROM
      tracks t1
    JOIN
      tracks t2 ON t1.album = t2.album
    WHERE
      t1.album != t2.album
      AND
      t1.song IN t1.album
...
def best_value
  ...
  execute(<<-SQL)
    SELECT
      albums.title, albums.price, COUNT(tracks.song) AS number_of_tracks
    FROM
      albums
    JOIN
      tracks ON albums.asin = tracks.album
