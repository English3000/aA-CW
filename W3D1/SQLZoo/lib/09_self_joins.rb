...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    JOIN
      stops stopb ON (b.stop_id = stopb.id)
    JOIN
      routes c ON (stopb.id = c.stop_id)
    JOIN
    ...
    WHERE
      ...
      stopb.name IN (
        ...
        FROM
          routes bb
        JOIN
          routes c ON (bb.company = c.company AND bb.num = c.num)
        JOIN
          stops stopbb ON (bb.stop_id = stopbb.id)
        JOIN
    ...
    JOIN
      routes c ON (
        a.company = c.company AND a.num = c.num AND
